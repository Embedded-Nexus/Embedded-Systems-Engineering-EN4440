...
void EcoWattUploader::checkRemoteConfig() {
  ...
  if (code == 200) {
    ...
    if (!err && doc["pending"]) {
      ...
      // 👇 Add these lines for visibility
      Serial.printf("[CFG] New sampling interval: %lu ms\n", (unsigned long)newCfg.acqPeriodMs);
      Serial.print("[CFG] New registers: ");
      for (auto &r : newCfg.registers) {
        Serial.print(r);
        Serial.print(" ");
      }
      Serial.println();
    ...
    Serial.printf("[CFG] No configuration update (HTTP %d)\n", code);
  }
  http.end();
}
