...
// ---- Utility helpers ----
static inline void put_u16_be(vector<uint8_t>& o, uint16_t v) {
    o.push_back((uint8_t)((v >> 8) & 0xFF));
    o.push_back((uint8_t)(v & 0xFF));
}

static inline uint16_t get_u16_be(const vector<uint8_t>& b, size_t i) {
...
static void put_varu(vector<uint8_t>& o, uint32_t v) {
    while (v >= 0x80) {
        o.push_back((uint8_t)(0x80 | (v & 0x7F)));
        v >>= 7;
    }
    o.push_back((uint8_t)v);
}

static uint32_t get_varu(const vector<uint8_t>& b, size_t& i) {
...
// ================================================================
// 🧮 Delta16VarCompressor Implementation
// ================================================================
vector<uint8_t> Delta16VarCompressor::compress(const vector<uint16_t>& values) {
    vector<uint8_t> out;
...
vector<uint16_t> Delta16VarCompressor::decompress(const vector<uint8_t>& blob) {
    vector<uint16_t> out;
...
BenchResult Delta16VarCompressor::benchmark(const vector<uint16_t>& values) {
    BenchResult br{};
    br.mode = name();
    br.samples = values.size();
    br.origBytes = values.size() * 2;
...
vector<uint8_t> TimeSeriesCompressor::compress(const vector<uint16_t>& values, int regs) {
    vector<uint8_t> out;
    if (regs <= 0 || values.empty()) return out;

    if (values.size() % (size_t)regs != 0) {
        // fallback to Delta16Var
    ...
    for (size_t f = 1; f < frames; ++f) {
        ...
        for (int j = 0; j < regs; j += 2) {
            uint8_t b = (uint8_t)((nibbles[j] << 4) |
                                  ((j + 1 < regs) ? (nibbles[j + 1] & 0x0F) : 0));
            out.push_back(b);
        }
...
vector<uint16_t> TimeSeriesCompressor::decompress(const vector<uint8_t>& blob, int regs) {
    vector<uint16_t> out;
...
BenchResult TimeSeriesCompressor::benchmark(const vector<uint16_t>& values, int regs) {
    BenchResult br{};
    br.mode = name();
    br.samples = values.size();
    br.origBytes = values.size() * 2;
